/**
 * High-level encrypt/decrypt API for the AES-256-GCM v1 suite.
 *
 * This is what most callers will import. It glues together:
 *   - header construction
 *   - chunk encryption/decryption with full AAD binding
 *   - the v1 wire format
 *
 * Suites 0x02 (XChaCha20-Poly1305) and 0x03 (PQ hybrid) provide identical
 * APIs in their own modules.
 */
...
/**
 * Encrypt a Blob/File with the AES-256-GCM v1 suite.
...
export async function encryptBlob(
  ...
  // Suite payload (caller-supplied or zero-filled placeholder)
  const suitePayload =
  ...
  if (suitePayload.length !== AES_GCM_V1_SUITE_PAYLOAD_LENGTH) {
    throw new RangeError(
      `encryptBlob: suitePayloadOverride must be ${AES_GCM_V1_SUITE_PAYLOAD_LENGTH} bytes`,
    )
  }
...
export interface DecryptOptions {
  ...
  onProgress?: ProgressCallback
  /**
   * If provided, the parser checks the suite matches before any AEAD
   * operation. Callers who want to refuse, e.g., legacy suites can pass
...
/**
 * Decrypt a v1 blob produced by `encryptBlob`. Reads the entire blob into
 * memory in chunked passes; for streaming decryption see `streams/`.
 */
export async function decryptBlob(options: DecryptOptions): Promise<Blob> {
  ...
  // Read enough to parse the header. The header is fixed-length plus a
  // bounded suite_payload (max 65535 bytes). We read up to 4 KiB and
  // fall back to streaming if the suite_payload is unusually large.
  ...
  try {
    ...
    if (err instanceof HeaderError && err.code === 'header_too_short') {
      // The probe might have been too small. Read the whole blob.
  ...
  if (parsed.suite !== SUITE.AES_256_GCM_V1) {
    throw new HeaderError(
      `decryptBlob: this entry point only supports aes-256-gcm-v1; got suite 0x${parsed.suite.toString(16)}`,
    )
  }
  ...
  for (let i = 0; i < parsed.totalChunks; i += 1) {
    ...
    // Sanity check: ciphertext for AES-GCM-v1 is exactly chunkSize+tag
    // for non-final chunks. Final chunk is between 1+tag and chunkSize+tag.
    const isFinal = i === parsed.totalChunks - 1
    const minCipher = 1 + AES_GCM_V1_TAG_BYTES
    const maxCipher = parsed.chunkSize + AES_GCM_V1_TAG_BYTES
